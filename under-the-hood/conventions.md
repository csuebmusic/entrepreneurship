...
- class notes/ holds a Markdown session plan for each live week (weekNN.md): the running order for the three-hour seminar, the recap of prior weeks, an account of the components already written, the discussion order for the readings, and the in-class exercises. Written in the second person, addressed to the student, under the student-facing style rules, so the instructor can speak or draft from the sentences directly. Bullets are used freely to organize. It links to the week's reading notes rather than repeating them. Written from week 3 forward; the two asynchronous weeks have none. No student information.
...
Each week has one HTML page, weekNN.html at the repo root, linked from Canvas. Weeks 7–8 share week07.html. The page is comprehensive but lean, and it guides rather than spoon-feeds: the reader should leave knowing what to read, what to notice, and what to make, and should still have to do the thinking.
